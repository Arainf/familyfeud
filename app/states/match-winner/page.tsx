...
export default function MatchWinnerPage() {
  ...
  if (!match || !gameData.tournament.teams) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-yellow-900 via-orange-900 to-yellow-700">
        <span className="text-white text-2xl animate-pulse">Match information unavailable.</span>
      </div>
    );
  }
  ...
  if (typeof team1Score !== "number" && typeof gameData.team1Score === "number") team1Score = gameData.team1Score;
  if (typeof team2Score !== "number" && typeof gameData.team2Score === "number") team2Score = gameData.team2Score;

  // Determine winner
  let winnerName = match.winnerName;
  if (!winnerName && typeof team1Score === "number" && typeof team2Score === "number") {
    winnerName = team1Score > team2Score ? team1?.name : team2?.name;
  }
  let winnerColor = "gray";
  if (winnerName && team1 && winnerName === team1.name) winnerColor = team1.color || team1.primaryColor || "gray";
  else if (winnerName && team2 && winnerName === team2.name) winnerColor = team2.color || team2.primaryColor || "gray";
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-yellow-900 via-orange-900 to-yellow-700 p-4">
      <div className="bg-black/70 rounded-3xl shadow-2xl p-12 flex flex-col items-center">
        <h1 className="text-4xl md:text-6xl font-bold text-yellow-300 mb-6 drop-shadow-lg animate-bounce">Match Winner!</h1>
        <div className="flex flex-col items-center gap-6">
          <div className={`w-24 h-24 rounded-full flex items-center justify-center text-6xl font-bold border-8 ${getColorClasses(winnerColor).bg} shadow-xl mb-4 animate-pulse`}></div>
          <span className="text-3xl md:text-5xl font-extrabold text-white tracking-wide animate-pulse">{winnerName || "TBD"}</span>
        </div>
        <div className="mt-8">
          <button
